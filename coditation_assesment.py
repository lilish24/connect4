# taking input from user for rows coloumn and pieces to connect
while True:
    row = int(input("Enter number of rows: "))
#checking row input for positive and non zero
    if row < 1:
...
#checking column input for positive and non zero
    if col < 1:
    ...
    def winner(self):
        ...
        for i in range(0, row, 2):
            if (directions[i][1] + directions[i+1][1] >= pi-1):
                ...
                print("Player {0} is the winner!".format(p))
                return lletter   
